    ...
    func loadCategories() async {
        ...
        if let token = apiService.authToken {
            do {
                ...
                AppLogger.general.info("ChatViewModel: User synced successfully")
            } catch {
                AppLogger.general.error("ChatViewModel: User sync failed: \(error.localizedDescription)")
            }
        }
        
        // トークンが無い場合はスキップ (認証監視により後で呼ばれる)
        guard let _ = apiService.authToken else {
             AppLogger.general.debug("ChatViewModel: Waiting for auth token to load categories...")
             return
        }
        ...
        do {
            ...
            AppLogger.general.error("Failed to load categories: \(error.localizedDescription)")
            // エラーでも初期値のまま続行
        }
    }
    ...
    func sendMessage(_ text: String, userId: String, tags: [String] = []) async {
        ...
        do {
            ...
            if let apiError = error as? APIError, case .forbidden(let detail) = apiError {
                 AppLogger.general.warning("Chat Limit Reached: \(detail)")
