...
struct LoginView: View {
    ...
    private func handleGoogleLogin() {
        ...
        AppLogger.auth.info("🔵 Google login started...")
        
        Task {
            do {
                AppLogger.auth.debug("🔵 Calling AuthService.signInWithGoogle()")
                let (user, token) = try await authService.signInWithGoogle()
                
                AppLogger.auth.info("✅ Google login success!")
                AppLogger.auth.debug("   User ID: \(user.id)")
                AppLogger.auth.debug("   Display Name: \(user.displayName ?? "nil")")
                AppLogger.auth.debug("   Email: \(user.email ?? "nil")")
                ...
                AppLogger.auth.error("❌ Google login failed: \(error)")
                await MainActor.run {
                    isLoading = false
    ...
    private func handleAppleLogin() {
        ...
        AppLogger.auth.info("🍎 Apple login started...")
        
        Task {
            do {
                AppLogger.auth.debug("🍎 Calling SignInWithAppleCoordinator")
                // Nonce生成
                ...
                AppLogger.auth.debug("🍎 Got authorization, signing in with Firebase...")
                let (user, token) = try await authService.signInWithApple(authorization: authorization, nonce: nonce)
                
                AppLogger.auth.info("✅ Apple login success!")
                AppLogger.auth.debug("   User ID: \(user.id)")
                ...
                AppLogger.auth.error("❌ Apple login failed: \(error)")
                await MainActor.run {
                    isLoading = false
                    errorMessage = error.localizedDescription
                }
    ...
    private func handleLineLogin() {
        ...
        AppLogger.auth.info("🟢 LINE login started...")
        
        Task {
            do {
                AppLogger.auth.debug("🟢 Calling LineAuthManager.login()")
                // LINE SDKでログイン
                let lineAccessToken = try await LineAuthManager.shared.login()
                
                AppLogger.auth.debug("🟢 Got LINE access token, authenticating with backend...")
                // バックエンド経由でFirebase認証
                let (user, token) = try await authService.signInWithLINE(lineAccessToken: lineAccessToken)
                
                AppLogger.auth.info("✅ LINE login success!")
                AppLogger.auth.info("   User ID: \(user.id)")
