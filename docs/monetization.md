# 収益化 & 価格設定 (Monetization & Pricing)

## プラン (Plans)

### FREE (梅)
- **価格(月額)**: 0円
- **価格(年間)**: 0円
- **チャット制限**: 10メッセージ / 日 (JST 0時リセット)。
- **音声アップロード制限**: 1ファイル / 日。
- **音声時間制限**: 1ファイルあたり15分 (超過分は切り捨て)。
- **月間音声処理枠**: 5時間 / 月。
- **ストレージ制限**: 5ファイル (ナレッジ)。

### STANDARD (竹)
- **価格(月額)**: 500円 / 月 |ios;600円 / 月
- **価格(年間)**: 5000円 / 年 |ios;6000円 / 年
- **チャット制限**: 100メッセージ / 日。
- **音声アップロード制限**: 無制限。
- **音声時間制限**: 1ファイルあたり90分。
- **月間音声処理枠**: 900分 (15時間) / 月。
- **ストレージ制限**: 200ファイル。

### PREMIUM (松)
- **価格(月額)**: 1800円 / 月 |ios;1900円 / 月
- **価格(年間)**: 18000円 / 年 |ios;19000円 / 年
- **チャット制限**: 200メッセージ / 日。
- **音声アップロード制限**: 無制限。
- **音声時間制限**: 1ファイルあたり180分 (3時間)。
- **月間音声処理枠**: 5400分 (90時間) / 月。
- **ストレージ制限**: 1000ファイル。

## アドオン (チケット)
- **音声チケット**: 100円
    - **+90分** の音声処理残高を追加。
    - 都度購入 (One-time)。

## リファラルプログラム (紹介制度)
- **仕組み**:
    - ユーザーがリンクを共有。
    - 被紹介者がサインアップ & 初回音声メモをアップロード。
    - **ステータス**: 被紹介者の初回アップロード時に「完了 (COMPLETED)」に更新。
- **キャンペーンモード (ローンチ時)**:
    - **被紹介者 (Referee) 特典**: STANDARDプラン 30日間 (フル機能)。
    - **紹介者 (Referrer) 特典**:
        - 初回成功: STANDARDプラン 30日間。
        - 2回目以降:音声チケット3枚
## 決済インフラ
- **プロバイダ**: Stripe
- **モード**: サブスクリプション (継続課金),  ペイメント (都度課金)。
- **Webhook**: サブスクリプション状態、トライアル適用、チケット付与を処理。
iosではrevenuecatを使って課金関連の整備
