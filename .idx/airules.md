...
あなたは以下の禁止事項とルールを順守するコーディングアシスタントです。
...
1.  **Previous Work**:
    `docs/` ディレクトリの内容を確認して、前回までの進捗を確認して欲しい。
2.  **Backend Logic Centralization**:
    「ルール（法律）はバックエンドが決めて、フロントエンド（警察）がそれに基づいて取り締まる」という原則に従うこと。
    制限ロジックや重要なビジネスロジックは、Next.js（BFF）ではなく、Pythonバックエンド（またはDB層）に集約し、フロントエンドはその結果（エラー等）をハンドリングする役割に徹すること。
3.  **コンテキストとアーキテクチャ確認:**
    * `docs/` ディレクトリ（および前回の進捗）を確認し、現状を把握してください。
    * **必ずルートディレクトリの `ARCHITECTURE.md` を読み込み、現在の実装計画がプロジェクトの設計思想に合致しているか確認してください。**
4.  **計画と意図の説明 (Plan & Intent):**
    * 「これから何を実装するか」
    * 「その意図は何か」
    * **「どのディレクトリのどのファイルを作成・変更するか（ARCHITECTURE.mdに基づく）」**
    * 「具体的な実装手順（擬似コードや設計案）」
    * これらを日本語で説明し、私の許可を求めてください。
5.  **実装 (Implementation):** 許可を得た後、コードを記述します。
...
## 4. 継続的な確認
* 次回以降のセッションでも、作業開始前には必ずこのルール (`.airules.md`) を再確認してください。

## 5. プロジェクト要件
* 実装の際は、ルートディレクトリにある `.agent/Requirement.md` の内容に基づいているか常に確認してください。また、都度 `Tasks.md` を確認して中身を更新してほしい。
...
### 6.1. 厳格な情報検索と検証
* **推測の禁止:** 自身のトレーニングデータ内の記憶だけで回答せず、事実確認が必要な質問（数値、歴史的事実、ニュース、技術仕様など）に対しては、必ずGoogle検索ツールを使用して最新かつ正確な情報を裏取りしてください。
...
## 7. コンテキスト認識の徹底
### 7.1 全体像把握
* 都度、全体像を把握するために、以下のファイルを必ず確認してください：
    * `.agent/` ディレクトリ配下の全ファイル
    * `.idx/` ディレクトリ配下の全ファイル
...
* **Dockerfile:** 本番用 (`runner` ステージ) は `NODE_ENV=production` となっているため、開発時には不適切。本番環境は現在CloudRunで運用されている。
...
## 9. アーキテクチャとディレクトリ構成 (Architecture Standards)
* **設計書の遵守:** ファイルの作成や移動を行う際は、必ず `ARCHITECTURE.md` で定義されたディレクトリ構造に従ってください。
