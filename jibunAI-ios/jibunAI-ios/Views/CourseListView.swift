    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                if courseViewModel.isLoading && courseViewModel.courses.isEmpty {
                    ProgressView()
                        .tint(.white)
                        .scaleEffect(1.5)
            ...
            .sheet(isPresented: $showingAddCourseFor) {
                AddCourseView(
                    ...
                    onSave: {
                        Task {
                             let success = await courseViewModel.createCourse(title: newCourseTitle, color: selectedColor, icon: nil)
                            if success {
        ...
        NavigationView {
            Form {
                Section(header: Text("コース情報")) {
                    ...
                    Text("\(title.count)/50")
                        .font(.caption)
                        .foregroundColor(title.count >= 50 ? .red : .gray)
