...
struct ChatView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 0) {
                    ...
                    HStack(spacing: 12) {
                        // カテゴリ選択
                        Menu {
                            ForEach(chatViewModel.categories, id: \.self) { category in
                        ...
                        HStack {
                            TextField("メッセージを入力...", text: $messageText)
                                .limitText($messageText, limit: 500) // 500文字制限
                                .foregroundColor(.white)
        ...
        .task {
            // カテゴリ（タグ）を最新化
            await chatViewModel.loadCategories()
        }
        .adaptivePaywallSheet(isPresented: $chatViewModel.showPaywall, subscriptionViewModel: subscriptionViewModel) {
             chatViewModel.showPaywall = false
        }
        .alert("利用上限に達しました", isPresented: $chatViewModel.showLimitAlert) {
            Button("プランを確認", role: .cancel) {
                chatViewModel.showPaywall = true
            }
        } message: {
    ...
    private func sendMessage() {
        ...
        Task {
            ...
            await chatViewModel.sendMessage(text, userId: appState.currentUser?.id ?? "unknown", tags: tags)
        }
    }
}
